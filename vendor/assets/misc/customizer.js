function loadColor(action, newColor){
    $.ajax({
        url: "/assets/customizer.php",
        method: "POST",
        cache: false,
        data: { action: action, new_color: newColor, default_color: defaultColor },
        success: function(results){
            if( action == "change_color" ){
                $('head').append( $('<link rel="stylesheet" type="text/css">').attr('href', '/assets/temporary-style.css') );
                $('link').each(function() {
...
function createCustomizer(defaultColor) {
    ...
    $.getScript( "assets/js/icheck.min.js", function( data, textStatus, jqxhr ) {
        if ($("input[type=checkbox]").length > 0) {
            ...
            $('.customizer #checkbox-rtl').on('ifChecked', function(event){
                $('head').append( $('<link rel="stylesheet" type="text/css">').attr('href', '/assets/rtl.css') );
                $("body").addClass("rtl");
    ...
    $('head').append( $('<link rel="stylesheet" type="text/css">').attr('href', '/assets/spectrum.css') );
